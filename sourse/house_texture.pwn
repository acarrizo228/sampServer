...
{
	b_tx_id, 		//Ид текстуры
	b_tx_number[32],  //Номер текстуры
	b_tx_model[32] 	//Модель текстуры
}
	...
	if( SelectedMenu[playerid] != INVALID_PARAM )
	{
		...
		if( PRESSED(KEY_WALK) )
		{
			...
			format:g_small_string( "\
				"W"Действие\t"W"Стоимость\n\
				"W"Купить текстуру\t"P"$%d\n\
				"W"Выйти из редактора", mprice );
			
			if(!SelectedType{playerid}) {
				SetPVarInt(playerid, "Hpanel:PriceTexture", mprice);
				D(playerid, D_HOUSE_TEXTURE, DIALOG_STYLE_TABLIST_HEADERS, "Ретекстуризация", g_small_string, "Выбрать", "Закрыть" );
			}
		}
	}
...
{
	SendOk(playerid,"Вы открыли редактор текстур. Используйте клавиши "P"F"G" и "P"H"G" - для управления," );
	SendOk(playerid,""P"ALT"G" - для покупки текстуры или выхода из редактора.");
